...
class AudioEngine:
    def __init__(self, ai_engine):
        ...
        self.active_samplerate = 16000
        self.active_channels = 1

    def _find_working_device(self):
        logger.info("Auto-calibrating microphones to find working input...")
        ...
        for i, dev in enumerate(devices):
            if dev['max_input_channels'] > 0:
                try:
                    ...
                    # Quick 0.5s test
                    recording = sd.rec(int(0.5 * samplerate), samplerate=samplerate, channels=channels, dtype='float32', device=i)
                    sd.wait()
                    max_amp = np.max(np.abs(recording))
                    if max_amp > 0.001:
                        logger.info(f"Selected working microphone: {dev['name']} (ID: {i})")
                        self.active_device = i
                        self.active_samplerate = samplerate
    ...
    def _record_audio(self, duration=4):
        recording = sd.rec(
            int(duration * self.active_samplerate), 
            samplerate=self.active_samplerate, 
            channels=self.active_channels, 
            dtype='int16', 
            device=self.active_device
        )
        sd.wait()
        return sr.AudioData(recording.tobytes(), self.active_samplerate, 2)

    def _listen_loop(self):
        try:
            # Test if sounddevice works and find the right channel
            if sd.query_devices() is None or len(sd.query_devices()) == 0:
                raise Exception("No audio devices found")
                
            has_mic = self._find_working_device()
            if not has_mic:
                raise Exception("All microphones reported silence (0 amplitude).")
